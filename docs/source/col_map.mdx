...
For all sentence transformers tasks, one needs to map columns to `sentence1`, `sentence2`, `sentence3` & `target` column.
...
```
{"sentence1": "premise", "sentence2": "hypothesis", "target": "label"}
```

## `pair_score`:

```
{"sentence1": "sentence1", "sentence2": "sentence2", "target": "score"}
```

## `triplet`:

```
{"sentence1": "anchor", "sentence2": "positive", "sentence3": "negative"}
```

## `qa`:

```
{"sentence1": "query", "sentence2": "answer"}
```
